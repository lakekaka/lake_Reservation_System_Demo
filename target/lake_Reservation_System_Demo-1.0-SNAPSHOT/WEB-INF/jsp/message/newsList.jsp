...
<body>
<div id="legend">
	<legend class="caption">通知公告</legend>
</div>
<c:choose>
	<c:when test="${page.curList != null && fn:length(page.curList) > 0 }">
		<c:forEach items="${page.curList }" var="news">
			<div>
				<a href="${pageContext.request.contextPath }/message/findNewsById?id=${news.id}">${news.title }</a>
				<div style="float: right;">${news.fillindate }</div><hr>
			</div>
		</c:forEach>
	</c:when>
	<c:otherwise>
...
<li><a href="${pageContext.request.contextPath }/message/dispatcherNewsList?thispage=1">首页</a></li>
    <li>
      <a href="${pageContext.request.contextPath }/message/dispatcherNewsList?thispage=${page.prePage }" aria-label="上一页">
    ...
    <c:forEach begin="${begin}" end="${end}" step="1" var="i">
		...
		<c:if test="${i!=page.thisPage}">	<!-- 其他页是超链接 -->
			<li><a href="${pageContext.request.contextPath }/message/dispatcherNewsList?thispage=${i }">${i }</a></li>
		</c:if>
	</c:forEach> 
    <li>
      <a href="${pageContext.request.contextPath }/message/dispatcherNewsList?thispage=${page.nextPage }" aria-label="下一页">	
        <span aria-hidden="true">&raquo;</span>
      </a>
    </li>
    <li><a href="${pageContext.request.contextPath }/message/dispatcherNewsList?thispage=${page.totalPage }">尾页</a></li>
